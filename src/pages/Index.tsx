      ...
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="grid md:grid-cols-2 gap-6">
          <Card className="p-6 bg-card border-border">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h2 className="text-xl font-semibold">Электрические характеристики</h2>
            </div>
            <div className="space-y-3 text-sm">
              <div className="flex justify-between p-2 bg-secondary/50 rounded">
                <span className="text-muted-foreground">Напряжение питания:</span>
                <span className="font-mono font-semibold">{powerSpecs.totalVoltage}</span>
              </div>
              <div className="flex justify-between p-2 bg-secondary/50 rounded">
                <span className="text-muted-foreground">Потребляемый ток:</span>
                <span className="font-mono font-semibold">{powerSpecs.totalCurrent}</span>
              </div>
              <div className="flex justify-between p-2 bg-secondary/50 rounded">
                <span className="text-muted-foreground">Источник питания:</span>
                <span className="font-mono font-semibold text-xs">{powerSpecs.powerSupply}</span>
              </div>
            </div>
          </Card>
          ...
          <Card className="p-6 bg-card border-border">
            ...
            <ul className="space-y-2 text-sm text-muted-foreground">
              <li className="flex gap-2">
                ...
                <span>DHT22 подключен к цифровому пину D2</span>
              </li>
              <li className="flex gap-2">
                <span className="text-primary">•</span>
                <span>LCD использует интерфейс I2C (A4, A5)</span>
              </li>
              <li className="flex gap-2">
                <span className="text-primary">•</span>
                <span>Реле управляется через пин D13</span>
              </li>
              <li className="flex gap-2">
                <span className="text-primary">•</span>
                <span>Потенциометр подключен к аналоговому входу A0</span>
              </li>
            </ul>
          </Card>
        </div>
        ...
        <Card className="p-6 bg-card border-border">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2 className="text-xl font-semibold">Спецификация компонентов</h2>
          </div>
          <div className="overflow-x-auto">
            <Table>
              <TableHeader>
                <TableRow className="border-border hover:bg-transparent">
                  <TableHead className="font-semibold">Компонент</TableHead>
                  <TableHead className="font-semibold">Напряжение</TableHead>
                  <TableHead className="font-semibold">Ток</TableHead>
                  <TableHead className="font-semibold">Подключение</TableHead>
              ...
              <TableBody>
                {components.map((component, index) => (
                  <TableRow key={index} className="border-border hover:bg-secondary/30">
                    <TableCell className="font-mono font-semibold">{component.name}</TableCell>
                    <TableCell className="font-mono text-primary">{component.voltage}</TableCell>
                    <TableCell className="font-mono">{component.current}</TableCell>
                    <TableCell className="font-mono text-xs">{component.pins}</TableCell>
                    <TableCell className="text-muted-foreground text-xs">{component.description}</TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
        ...
        <Card className="p-6 bg-card border-border">
          <div className="flex items-center gap-2 mb-4">
            <Icon name="Cable" size={20} className="text-primary" />
            <h2 className="text-xl font-semibold">Таблица соединений</h2>
          </div>
          <div className="overflow-x-auto">
            <Table>
              <TableHeader>
                <TableRow className="border-border hover:bg-transparent">
                  <TableHead className="font-semibold">Откуда</TableHead>
                  <TableHead className="font-semibold">Куда</TableHead>
                  <TableHead className="font-semibold">Цвет провода</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {connections.map((conn, index) => (
                  <TableRow key={index} className="border-border hover:bg-secondary/30">
                    <TableCell className="font-mono text-sm">{conn.from}</TableCell>
                    <TableCell className="font-mono text-sm">{conn.to}</TableCell>
                    <TableCell>
                      <div className="flex items-center gap-2">
                        <div className={`w-4 h-4 rounded ${conn.color} border border-border`}></div>
                        <span className="text-xs font-mono">{conn.color.replace('bg-', '').replace('-500', '').replace('-700', '')}</span>
                      </div>
                    </TableCell>
                  </TableRow>
                ))}
